...
///   long const hexhex_exp_n_dsz_tris = 12 ;
///   long const hexhex_exp_n_tsz_tets = 6 ;
///   long const hexhex_exp_tri_per_pair = hexhex_exp_n_dsz_tris * hexhex_exp_n_tsz_tets ;
///   long ipair   = itri / hexhex_exp_tri_per_pair ;
///   int dfacet  = ( itri / hexhex_exp_n_tsz_tets ) % hexhex_exp_n_dsz_tris ;
///   int ttet    = itri % hexhex_exp_n_tsz_tets ;
///   long pair_base_thr = ipair * hexhex_exp_tri_per_pair ;
...
///   hex_intsc_subz_exp ( xds, xts, dfacet, ttet, vv, vx, vy, vz ) ;
///
///   vv_out[4*ipair+0]  = vv ;
///   vv_out[4*ipair+1]  = vx ;
///   vv_out[4*ipair+2]  = vy ;
...
///
///      cuda_intsc_tri_tet_exp
///       ( xdt, ydt, zdt, xtt, ytt, ztt, vv, vx, vy, vz ) ;
///    }


#ifndef RAJAPerf_Apps_INTSC_HEXHEX_EXP_HPP
#define RAJAPerf_Apps_INTSC_HEXHEX_EXP_HPP
...
static Index_type constexpr hexhex_exp_gpu_block_size = 64 ;

// Number of donor triangles in a 12 sided hexahedron subzone
static Index_type constexpr hexhex_exp_n_dsz_tris = 12 ;

// Number of target tets in a 12 sided hexahedron subzone
static Index_type constexpr hexhex_exp_n_tsz_tets = 6 ;

// Number of triangle-tet contributions per subzone pair (= 72)
static Index_type constexpr hexhex_exp_tri_per_pair = hexhex_exp_n_dsz_tris * hexhex_exp_n_tsz_tets ;

// A standard intersection is defined as eight subzone pairs.
static Index_type constexpr hexhex_exp_npairs_per_std_intsc = 8 ;

// Number of subzone pairs grouped together in fixup.
//  (is hexhex_exp_gpu_block_size / gcd(hexhex_exp_gpu_block_size, hexhex_exp_tri_per_pair)
static Index_type constexpr hexhex_exp_fixup_groupsize = 8 ;

// 576 triangle contributions per group (eight subzone intersections grouped)
static Index_type constexpr hexhex_exp_tri_per_group =
    hexhex_exp_tri_per_pair * hexhex_exp_fixup_groupsize ;

// 576 threads (triangle contributions) per standard intersection.
static Index_type constexpr hexhex_exp_tri_per_std_intsc =
    hexhex_exp_tri_per_pair * hexhex_exp_npairs_per_std_intsc ;

// Compute four values per pair (intersection volume, x, y, z moments).
static Index_type constexpr hexhex_exp_nvals_per_pair = 4 ;

// Number of values computed per standard intersection ( = 4*8 = 32)
static Index_type constexpr hexhex_exp_nvals_per_std_intsc =
    hexhex_exp_nvals_per_pair * hexhex_exp_npairs_per_std_intsc ;

// 9 gpu blocks per group of intersections of subzones
static Index_type hexhex_exp_blks_per_group = hexhex_exp_tri_per_group / hexhex_exp_gpu_block_size ;

// Minimum warp size (is 32 for Cuda and 64 for Hip).
static Index_type constexpr hexhex_exp_min_warp_size = 32 ;

// Maximum warps per block (= 2)
static Index_type constexpr hexhex_exp_max_warps_per_block =
    (hexhex_exp_gpu_block_size + hexhex_exp_min_warp_size - 1) / hexhex_exp_min_warp_size ;

// Maximum number of pairs represented in a gpu block.
// With hexhex_exp_gpu_block_size < hexhex_exp_tri_per_pair at most two pairs represented in a block.
//
static Index_type constexpr hexhex_exp_max_pairs_per_block = 2 ;

// Number of intermediate computed values per gpu block
static Index_type constexpr hexhex_exp_n_vvint_per_block =
    hexhex_exp_max_pairs_per_block * hexhex_exp_nvals_per_pair ;

// 72 data entries in group in intermediate results (before fixup).
static Index_type hexhex_exp_n_vvint_per_group = hexhex_exp_n_vvint_per_block * hexhex_exp_blks_per_group ;
...
// hexhex_exp_len_vv_reduce = 16.
static Index_type constexpr hexhex_exp_len_vv_reduce =
    hexhex_exp_max_warps_per_block * hexhex_exp_max_pairs_per_block * hexhex_exp_nvals_per_pair ;

}  // end namespace rajaperf

#define  INTSC_HEXHEX_EXP_DATA_SETUP  \
  Real_ptr const dsubz  = m_dsubz ;  \
  Real_ptr const tsubz  = m_tsubz ;  \
  Real_ptr       vv_int = m_vv_int ; \
  Real_ptr      vv_pair = m_vv_out ;

#include "INTSC_HEXHEX_EXP_BODY.hpp"
...
class INTSC_HEXHEX_EXP : public KernelBase
{
public:

  INTSC_HEXHEX_EXP(const RunParams& params);
