...
void ZONAL_ACCUMULATION_3D::setCountedAttributes()
{
...

  {
    ...
    RAJAPERF_COUNTERS_REP_SCOPE()
    {
      ...
      RAJAPERF_COUNTERS_PAR_LOOP(for (Index_type ii = ibegin ; ii < iend ; ++ii )) {
        RAJAPERF_COUNTERS_LOOP_BODY(ZONAL_ACCUMULATION_3D_BODY_INDEX);
        RAJAPERF_COUNTERS_LOOP_BODY(ZONAL_ACCUMULATION_3D_BODY);
      }
