...
__global__ void intsc_hexhex_exp_hip
...
template < Size_type block_size >
__global__ void intsc_hexhex_exp_hip_fixup_vv_64to72
...
template < Size_type block_size >
void INTSC_HEXHEX_EXP::runHipVariantImpl(VariantID vid)
{
  ...
  if ( do_warmup ) {
    ...
    RPlaunchHipKernel( (intsc_hexhex_exp_hip<block_size>),
                       grid_size1, block_size,
                       shmem, res.get_stream(),
                       m_dsubz, m_tsubz,
                       0UL, m_vv_int ) ;
    RPlaunchHipKernel( (intsc_hexhex_exp_hip_fixup_vv_64to72<block_size>),
                       grid_size1, block_size,
                       shmem, res.get_stream(),
  ...
  if ( vid == Base_HIP ) {
    ...
    Real_ptr dsubz_blk;
    Real_ptr tsubz_blk;
    allocData(DataSpace::HipDevice, dsubz_blk, stage2_aosoa_len);
    allocData(DataSpace::HipDevice, tsubz_blk, stage2_aosoa_len);
    ...
    for (RepIndex_type irep = 0; irep < run_reps; RP_REPCOUNTINC(irep)) {
      ...
      RPlaunchHipKernel( (stage2_subz_aos_to_aosoa_pair_kernel<
...
#undef LAUNCH_TARGET_TET
    ...
    deallocData(DataSpace::HipDevice, dsubz_blk);
    deallocData(DataSpace::HipDevice, tsubz_blk);
    ...
    for (RepIndex_type irep = 0; irep < run_reps; RP_REPCOUNTINC(irep)) {
      ...
      auto intsc_hexhex_lambda = [=] __device__ ( Index_type i ) {
          ...
          __shared__ Real_type vv_reduce[hexhex_exp_len_vv_reduce] ;  // hexhex_exp_len_vv_reduce=16
    ...
    for (RepIndex_type irep = 0; irep < run_reps; RP_REPCOUNTINC(irep)) {
      ...
      RAJA::forall< RAJA::hip_exec<stage2_transpose_block_size,
                                   true /*async*/> >( res,
        RAJA::RangeSegment(
...
#define RAJA_LAUNCH_TARGET_TET(TTET)                                          \
      RAJA::forall< RAJA::hip_exec<stage2_tile_size,                          \
                                   true /*async*/> >( res,                    \
...
#undef RAJA_LAUNCH_TARGET_TET
    ...
    deallocData(DataSpace::HipDevice, tsubz_blk);
     ...
     getCout() << "\n  INTSC_HEXHEX_EXP : Unknown Hip variant id = " << vid << std::endl;
  }
}

RAJAPERF_GPU_BLOCK_SIZE_TUNING_DEFINE_BOILERPLATE(INTSC_HEXHEX_EXP, Hip, Base_HIP, Lambda_HIP, RAJA_HIP)
