  ...
  case Base_HIP: {
    ...
    if (tune_idx == 0) {
      ...
      for (RepIndex_type irep = 0; irep < run_reps; irep = irep + 1) {
        ...
        RPlaunchHipKernel((MassVec3DPA_RUNTIME_LOOP_INC<block_size>), NE,
                          nthreads_per_block, shmem, res.get_stream(), B, Bt, D,
    ...
    if (tune_idx == 1) {
      ...
      for (RepIndex_type irep = 0; irep < run_reps; irep = irep + 1) {
        ...
        RAJA::launch<launch_policy>(
            ...
            [=] RAJA_HOST_DEVICE(RAJA::LaunchContext ctx) {
              RAJA::loop<outer_x>(
                  ctx, RAJA::RangeSegment(0, NE),
                  [&](Index_type e) {
                    MASSVEC3DPA_0_GPU

                    RAJA::loop<inner_z>(
                        ctx, RAJA::RangeSegment(0, 1), [&](Index_type) {
                          RAJA::loop<inner_y>(
                              ctx, RAJA::RangeSegment(0, MVPA_D1D),
                              [&](Index_type d) {
                                RAJA::loop<inner_x>(
                                    ctx, RAJA::RangeSegment(0, MVPA_Q1D),
                                    [&](Index_type q) { MASSVEC3DPA_1; });
                              });
                        });
