...
void MASSVEC3DPA::runCudaVariantImpl(VariantID vid, size_t tune_idx)
{
  ...
  case Base_CUDA: {
    ...
    if (tune_idx == 0) {
      ...
      for (RepIndex_type irep = 0; irep < run_reps; irep = irep + 1) {
        ...
        RPlaunchCudaKernel((MassVec3DPA_BLOCKDIM_COPY_LOOP_INC<block_size>), NE,
                           nthreads_per_block, shmem, res.get_stream(), B, Bt, D,
                           X, Y, static_cast<Index_type>(MVPA_Q1D));
      }
      stopTimer();
  ...
  case RAJA_CUDA: {
    ...
    if (tune_idx == 1) {
      ...
      using inner_x = RAJA::LoopPolicy<RAJA::cuda_ctx_thread_loop_x>;

      using inner_y = RAJA::LoopPolicy<RAJA::cuda_ctx_thread_loop_y>;

      using inner_z = RAJA::LoopPolicy<RAJA::cuda_ctx_thread_loop_z>;
