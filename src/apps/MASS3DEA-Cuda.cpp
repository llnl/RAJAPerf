...
__global__ void Mass3DEA(const Real_ptr B, const Real_ptr D, Real_ptr M) {
  ...
  GPU_FOREACH_THREAD(iz, z, 1) {
    GPU_FOREACH_THREAD(d, x, mea::MEA_D1D) {
      GPU_FOREACH_THREAD(q, y, mea::MEA_Q1D) {
        MASS3DEA_1
      }
    }
  }
  ...
  GPU_FOREACH_THREAD(k1, x, mea::MEA_Q1D) {
    GPU_FOREACH_THREAD(k2, y, mea::MEA_Q1D) {
      GPU_FOREACH_THREAD(k3, z, mea::MEA_Q1D) {
        MASS3DEA_3
      }
    }
  }
  ...
  GPU_FOREACH_THREAD(i1, x, mea::MEA_D1D) {
    GPU_FOREACH_THREAD(i2, y, mea::MEA_D1D) {
      GPU_FOREACH_THREAD(i3, z, mea::MEA_D1D) {
        MASS3DEA_4
      }
    }
  }
...
void MASS3DEA::runCudaVariantImpl(VariantID vid) {
  ...
  case Base_CUDA: {
    ...
    for (RepIndex_type irep = 0; irep < run_reps; RP_REPCOUNTINC(irep)) {
      ...
      dim3 nthreads_per_block(mea::MEA_D1D, mea::MEA_D1D, mea::MEA_D1D);
  ...
  case RAJA_CUDA: {
    ...
    using launch_policy = RAJA::LaunchPolicy<RAJA::cuda_launch_t<async, mea::MEA_D1D*mea::MEA_D1D*mea::MEA_D1D>>;

    using outer_x = RAJA::LoopPolicy<RAJA::cuda_block_x_direct>;

    using inner_x = RAJA::LoopPolicy<RAJA::cuda_thread_size_x_loop<mea::MEA_D1D>>;

    using inner_y = RAJA::LoopPolicy<RAJA::cuda_thread_size_y_loop<mea::MEA_D1D>>;

    using inner_z = RAJA::LoopPolicy<RAJA::cuda_thread_size_z_loop<mea::MEA_D1D>>;
    ...
    for (RepIndex_type irep = 0; irep < run_reps; RP_REPCOUNTINC(irep)) {
      ...
      RAJA::launch<launch_policy>( res,
        RAJA::LaunchParams(RAJA::Teams(NE),
                         RAJA::Threads(mea::MEA_D1D, mea::MEA_D1D, mea::MEA_D1D)),
