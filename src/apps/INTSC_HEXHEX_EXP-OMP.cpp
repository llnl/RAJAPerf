...
void INTSC_HEXHEX_EXP::runOpenMPVariant(VariantID vid)
{
  ...
  const Index_type iend = RAJA_DIVIDE_CEILING_INT(n_subz_intsc, hexhex_exp_fixup_groupsize);

  INTSC_HEXHEX_EXP_DATA_SETUP ;

  auto intsc_hexhex_lam = [=] ( Index_type i ) {
      INTSC_HEXHEX_EXP_OMP ( i, iend ) ;
  } ;
...
#pragma omp parallel for
  ...
  switch ( vid ) {
    ...
    default : {
      getCout() << "\n  INTSC_HEXHEX_EXP : Unknown OpenMP variant id = " << vid << std::endl;
    }
...
RAJAPERF_DEFAULT_TUNING_DEFINE_BOILERPLATE(INTSC_HEXHEX_EXP, OpenMP, Base_OpenMP, Lambda_OpenMP, RAJA_OpenMP)
