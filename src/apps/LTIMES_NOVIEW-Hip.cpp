...
#define LTIMES_NOVIEW_BLOCK_MOMENTS_THREADS_PER_BLOCK_TEMPLATE_PARAMS_HIP \
  block_moments_m_block_sz, block_moments_g_block_sz, block_moments_z_block_sz

#define LTIMES_NOVIEW_BLOCK_MOMENTS_THREADS_PER_BLOCK_HIP \
  dim3 nthreads_per_block(LTIMES_NOVIEW_BLOCK_MOMENTS_THREADS_PER_BLOCK_TEMPLATE_PARAMS_HIP); \
  static_assert(block_moments_m_block_sz*block_moments_g_block_sz*block_moments_z_block_sz == block_size, "Invalid block_size");

#define LTIMES_NOVIEW_BLOCK_MOMENTS_NBLOCKS_HIP \
...
#define LTIMES_NOVIEW_FACTORIZED_THREADS_PER_BLOCK_TEMPLATE_PARAMS_HIP \
  factorized_m_block_sz, factorized_g_block_sz, factorized_z_block_sz

#define LTIMES_NOVIEW_FACTORIZED_THREADS_PER_BLOCK_HIP \
  dim3 nthreads_per_block(LTIMES_NOVIEW_FACTORIZED_THREADS_PER_BLOCK_TEMPLATE_PARAMS_HIP); \
  static_assert(factorized_m_block_sz*factorized_g_block_sz*factorized_z_block_sz == block_size, "Invalid block_size");

#define LTIMES_NOVIEW_FACTORIZED_NBLOCKS_HIP \
  ...
  if ( vid == Base_HIP ) {
    ...
    for (RepIndex_type irep = 0; irep < run_reps; RP_REPCOUNTINC(irep)) {
      ...
      if constexpr (tune_idx == 1) {
        LTIMES_NOVIEW_FACTORIZED_THREADS_PER_BLOCK_HIP;
        LTIMES_NOVIEW_FACTORIZED_NBLOCKS_HIP;

        RPlaunchHipKernel(
          (ltimes_noview_factorized<LTIMES_NOVIEW_FACTORIZED_THREADS_PER_BLOCK_TEMPLATE_PARAMS_HIP>),
          nblocks, nthreads_per_block,
          shmem, res.get_stream(),
          phidat, elldat, psidat,
          num_d, num_m, num_g, num_z );
      } else {
        LTIMES_NOVIEW_BLOCK_MOMENTS_THREADS_PER_BLOCK_HIP;
        LTIMES_NOVIEW_BLOCK_MOMENTS_NBLOCKS_HIP;
    ...
    for (RepIndex_type irep = 0; irep < run_reps; RP_REPCOUNTINC(irep)) {
      ...
      if constexpr (tune_idx == 1) {
        ...
        RPlaunchHipKernel(
          (ltimes_noview_lam_factorized<LTIMES_NOVIEW_FACTORIZED_THREADS_PER_BLOCK_TEMPLATE_PARAMS_HIP,
