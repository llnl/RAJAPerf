...
INTSC_HEXHEX_EXP::INTSC_HEXHEX_EXP(const RunParams& params)
  : KernelBase(rajaperf::Apps_INTSC_HEXHEX_EXP, params)
{
...
void INTSC_HEXHEX_EXP::setSize(Index_type target_size, Index_type target_reps)
{
  ...
  m_n_subz_intsc = hexhex_exp_npairs_per_std_intsc * m_n_std_intsc ;

  const Int_type block_size = default_gpu_block_size ;
  m_nthreads = hexhex_exp_tri_per_pair * m_n_subz_intsc ;  // 72 threads per subzone pair
  ...
  setBytesAllocatedPerRep( 1*sizeof(Real_type) * 24L*m_n_subz_intsc + // dsubz
                           ...
                           1*sizeof(Real_type) * hexhex_exp_n_vvint_per_block * m_gsize + // vv_int
                           1*sizeof(Real_type) * hexhex_exp_nvals_per_pair * m_n_subz_intsc + // vv_out
                           1*sizeof(Real_type) * hexhex_exp_nvals_per_pair * m_n_subz_intsc ); // vv
  ...
  constexpr Size_type flops_per_intsc = flops_per_tri * hexhex_exp_tri_per_std_intsc ;

  setFLOPsPerRep(m_n_std_intsc * flops_per_intsc);
}

INTSC_HEXHEX_EXP::~INTSC_HEXHEX_EXP()
{
}


void INTSC_HEXHEX_EXP::setUp(VariantID vid,
                         Size_type RAJAPERF_UNUSED_ARG(tune_idx))
{
  ...
  allocData ( m_vv_int, hexhex_exp_n_vvint_per_block * m_gsize, vid ) ;

  allocAndInitDataConst ( m_vv_out, hexhex_exp_nvals_per_pair * m_n_subz_intsc, 0.0, vid ) ;

  // output volumes and moments on the host
  allocData ( DataSpace::Host, m_vv, hexhex_exp_nvals_per_pair * m_n_subz_intsc ) ;
}


//   Number of subzone intersections = 8 * number of standard intersections.
//
void INTSC_HEXHEX_EXP::check_intsc_volume_moments(
    Real_const_ptr vv,   // computed volumes, moments on the host
    VariantID vid)       // Print variant name in case of error
{

  {
    Char_const_ptr tst = "INTSC_HEXHEX_EXP:" ;
    ...
    for ( Index_type k = 0 ; k < m_n_subz_intsc ; ++k ) {
      ...
      Real_type dv  = vv[ hexhex_exp_nvals_per_pair*k + 0 ] - v0 ;
      Real_type dxm = vv[ hexhex_exp_nvals_per_pair*k + 1 ] - vx ;
      Real_type dym = vv[ hexhex_exp_nvals_per_pair*k + 2 ] - vy ;
      Real_type dzm = vv[ hexhex_exp_nvals_per_pair*k + 3 ] - vz ;
      ...
      if ( ( dv*dv   > tolsqv ) ||
        ...
        show_comparison ( k, "vv", vv[hexhex_exp_nvals_per_pair*k+0], v0, sqrt(tolsqv) ) ;
        show_comparison ( k, "vx", vv[hexhex_exp_nvals_per_pair*k+1], vx, sqrt(tolsqx) ) ;
        show_comparison ( k, "vy", vv[hexhex_exp_nvals_per_pair*k+2], vy, sqrt(tolsqy) ) ;
        show_comparison ( k, "vz", vv[hexhex_exp_nvals_per_pair*k+3], vz, sqrt(tolsqz) ) ;
        getCout() << std::endl ;
...
void INTSC_HEXHEX_EXP::updateChecksum(VariantID vid,
                                  size_t RAJAPERF_UNUSED_ARG(tune_idx))
{
  copyData ( DataSpace::Host, m_vv,
             getDataSpace(vid), m_vv_out, hexhex_exp_nvals_per_pair*m_n_subz_intsc ) ;

  check_intsc_volume_moments ( m_vv, vid ) ;

  addToChecksum(m_vv_out, hexhex_exp_nvals_per_pair*m_n_subz_intsc, vid);
}

void INTSC_HEXHEX_EXP::tearDown(VariantID vid,
                            Size_type RAJAPERF_UNUSED_ARG(tune_idx))
{
