...
#define DPA_d(qx, qy, qz, s, e)                                                    \
  D[qx + DPA_Q1D * qy + DPA_Q1D * DPA_Q1D * qz + DPA_Q1D * DPA_Q1D * DPA_Q1D * s  +  DPA_Q1D * DPA_Q1D * DPA_Q1D * DPA_SYM * e]
...
#define DIFFUSION3DPA_2 \
        const Index_type i = DPA_qi(qx,dy,DPA_Q1D); \
        const Index_type j = DPA_dj(qx,dy,DPA_D1D); \
        const Index_type k = DPA_qk(qx,dy,DPA_Q1D); \
        const Index_type l = DPA_dl(qx,dy,DPA_D1D); \
        B[i][j] = DPA_b(qx,dy); \
        G[k][l] = DPA_g(qx,dy) * DPA_sign(qx,dy); \
...
#define DIFFUSION3DPA_3 \
           Real_type u = 0.0, v = 0.0; \
            ...
            { \
               const Index_type i = DPA_qi(qx,dx,DPA_Q1D); \
               const Index_type j = DPA_dj(qx,dx,DPA_D1D); \
               const Index_type k = DPA_qk(qx,dx,DPA_Q1D); \
               const Index_type l = DPA_dl(qx,dx,DPA_D1D); \
               const Real_type s = DPA_sign(qx,dx); \
...
#define DIFFUSION3DPA_4 \
   ...
   { \
      const Index_type i = DPA_qi(qy,dy,DPA_Q1D); \
      const Index_type j = DPA_dj(qy,dy,DPA_D1D); \
      const Index_type k = DPA_qk(qy,dy,DPA_Q1D); \
      const Index_type l = DPA_dl(qy,dy,DPA_D1D); \
      const Real_type s = DPA_sign(qy,dy); \
      u += DDQ1[dz][dy][qx] * B[i][j]; \
      v += DDQ0[dz][dy][qx] * G[k][l] * s; \
      w += DDQ0[dz][dy][qx] * B[i][j]; \
   } \
...
#define DIFFUSION3DPA_5 \
               ...
               { \
                  const Index_type i = DPA_qi(qz,dz,DPA_Q1D); \
                  const Index_type j = DPA_dj(qz,dz,DPA_D1D); \
                  const Index_type k = DPA_qk(qz,dz,DPA_Q1D); \
                  const Index_type l = DPA_dl(qz,dz,DPA_D1D); \
                  const Real_type s = DPA_sign(qz,dz); \
                  u += DQQ0[dz][qy][qx] * B[i][j]; \
                  v += DQQ1[dz][qy][qx] * B[i][j]; \
                  w += DQQ2[dz][qy][qx] * G[k][l] * s; \
               } \
...
#define DIFFUSION3DPA_6 \
               const Index_type i = DPA_qi(q,d,DPA_Q1D); \
               const Index_type j = DPA_dj(q,d,DPA_D1D); \
               const Index_type k = DPA_qk(q,d,DPA_Q1D); \
               const Index_type l = DPA_dl(q,d,DPA_D1D); \
               Bt[j][i] = DPA_b(q,d); \
               Gt[l][k] = DPA_g(q,d) * DPA_sign(q,d);
